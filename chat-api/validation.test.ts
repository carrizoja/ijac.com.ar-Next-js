...
describe("grounded output validation", () => {
  ...
  it("rejects a token-preserving subject/object role inversion", () => reject({ supported: true, answer: "Business technology environments provides managed IT support for iJAC.", language: "en", sources: [{ id: "managed-it-support", title: "Managed IT support" }] }));
  it("accepts a meaning-preserving subject-led paraphrase", () => {
    const result = validateGroundedOutput({ supported: true, answer: "iJAC provides managed IT support to business technology environments.", language: "en", sources: [{ id: "managed-it-support", title: "Managed IT support" }] }, supportEvidence, "en");
    expect(result.valid).toBe(true);
    expect(result.response.code).toBe("SUCCESS");
  });
